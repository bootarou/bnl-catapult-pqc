...
**✅ 済: config 化とプリミティブの depth パラメータ化**（コミット `0bbd076d5`）
- `[chain] iVrfTreeDepth`（既定16）と `iVrfActivationDelay`（既定0）をオプショナル追加。
- proof は `iVrf_Max_Tree_Depth`(32)＝**固定1056B**を確保し、config depth を変えても
  ブロックヘッダ形式は不変（先頭 depth 個のみ有効・残りゼロ詰め）。
- C++/SDK とも depth パラメータ化、depth10 で byte 一致を再確認。
